...
#-------------------------------------------------------------------------------
# Target Parameters ----
n_sites  <- 500 #100 can reduce to 100 sites, 500 dates if we want to see the strength of the ICAR in areas without sites. Otherwise 200 sites, 800 dates (reflects density of real data)
n_dates  <- 2000 #500
origin_point <- st_sfc(st_point(c(5, 60)), crs=4326) #dispersal origin point #top left: c(-11, 72.5)
...
n_areas <- length(num) #number of regions.
...
#For example: A forest and water present in the sample window
...
y1 <- ggplot(data = hex_area_win) +
  geom_sf(data = st_as_sf(sampling_win, crs = 4326), aes(color = "grey50")) + #sampling window with coastal buffer
...
sim_constants$mu <- rep(2000, n_areas)
sim_constants$mu2 <- rep(500, n_areas) 
...
#Output
pdf(file=here('output','figures','figure1_woa_sim.pdf'), width=15, height=8)
