...
theta_init = medCal(calibrate(d1$cra, 
                              d1$cra_error, 
                              verbose = FALSE))
...
theta_init  <-  medCal(calibrate(d2$cra,
                                 d2$cra_error,
                                 verbose = FALSE))
dd  <-  data.frame(theta=theta_init, id=constants2$id_sites)
buffer  <- 100
...
diff_age  <- earliest$theta - latest$theta
delta_init  <- diff_age + buffer
alpha_init  <- earliest$theta + buffer/2
