...
gpqr_tau99 <- coda::mcmc.list(chain_output)
rhat.t99 <- coda::gelman.diag(gpqr_tau99, multivariate = F)
# range(rhat.t99$psrf[,1])
# ii  <- which(rhat.t99$psrf[,1]>1.01)  
ess.t99  <- coda::effectiveSize(gpqr_tau99)
# range(ess.t99)


#-------------------------------------------------------------------------------
## Store Output ----
save(rhat.t99, ess.t99, gpqr_tau99, file=here('output', 'gpqr_tau99.RData'))